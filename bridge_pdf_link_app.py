...
v8: PaddleOCR対応・DPI150標準

起動方法:
    python bridge_pdf_link_app.py

必要ライブラリ:
    pip install pikepdf pypdf pdf2image pillow paddleocr tkinterdnd2
    ※ tkinterdnd2 はドラッグ＆ドロップ用（任意。なくても動作します）
    ※ PaddleOCR は初回起動時にモデルを自動ダウンロードします
"""
...
def _get_paddle_ocr():
    """PaddleOCRのシングルトンインスタンスを返す（遅延初期化）。
    PaddleOCR v3系 API に対応（use_angle_cls/show_log 廃止済み）。
    PyInstaller同梱時はモデルソース確認をスキップする。
    """
    global _paddle_ocr
    if _paddle_ocr is None:
        try:
            ...
            # PyInstaller同梱時: モデルホスト疎通チェックをスキップ
            os.environ.setdefault("PADDLE_PDX_DISABLE_MODEL_SOURCE_CHECK", "True")
            from paddleocr import PaddleOCR
            try:
                # v3系: use_textline_orientation で縦書き・斜め対応
...
def _preprocess_for_ocr(img):
    """
    OCR精度向上のための画像前処理。
    グレースケール変換 → コントラスト強調 → シャープネス強調 → 大津法二値化
    numpyなしでPillowのみで実装。
    """
    ...
    # 大津法によるしきい値算出（numpyなし・Pillowのhistogram使用）
    hist = gray.histogram()   # 256要素のリスト
    total = gray.width * gray.height
    sum_all = sum(t * hist[t] for t in range(256))
    sum_b, w_b, max_var, thresh = 0.0, 0, 0.0, 128
    for t in range(256):
        w_b += hist[t]
        if w_b == 0:
            continue
    ...
    binarized = gray.point(lambda p: 255 if p > thresh else 0, '1')
    return binarized.convert('RGB')  # PaddleOCRはRGB入力を受け付ける


def _run_paddle_ocr(img):
    """
    PaddleOCRでPIL Imageからテキストを抽出して返す。
    PaddleOCRはnumpy配列またはファイルパスを受け付けるため、
    一時的にnumpy配列へ変換して渡す。
    """
    import numpy as np
    ocr = _get_paddle_ocr()
...
def ocr_diagram_page(pdf_path, page_idx, dpi):
    """
    損傷図ページをPaddleOCRでOCRして (span, photo_num) ペアのセットを返す。
    """
    imgs = convert_pdf(pdf_path, dpi=dpi,
                       first_page=page_idx + 1, last_page=page_idx + 1)
    if not imgs:
        return set()
    processed = _preprocess_for_ocr(imgs[0])
    t = _run_paddle_ocr(processed)
...
def run_process(input_path, output_path, dpi, log_cb, done_cb):
    ...
    try:
        # PaddleOCR 初期化
        log_cb("PaddleOCR を初期化中...")
        ocr = _get_paddle_ocr()
        if ocr is None:
            raise RuntimeError(
                "PaddleOCR が見つかりません。\n"
                "pip install paddleocr でインストールしてください。")
        log_cb("PaddleOCR: 初期化完了")
        ...
        for didx in diag_pages:
            ...
            # 2) テキスト抽出で取れなかった場合のみPaddleOCR
            if not pairs:
                log_cb(f"  損傷図 p.{didx+1}: テキスト抽出で取れず、PaddleOCRを実行...")
        ...
        for didx, pairs in diagram_to_photo_keys.items():
            ...
            # OCRで写真番号が取れなかった場合はボタンを作らない
            if not diag_to_photo_pages[didx] and page_span is not None:
                log_cb(f"  警告: 損傷図 p.{didx+1} の写真番号が取得できませんでした。ボタンは追加しません。")
        ...
        for didx, plist in diag_to_photo_pages.items():
            ...
            for pp in plist:
                ...
                # リンク先の損傷写真ページの全写真番号を使ってラベルを作成
                page_nums = photo_page_nums.get(pp, [])
                if page_nums:
                    if is_multi_span and span:
                        label = f"{span}-{min(page_nums)}〜{max(page_nums)}" if len(page_nums) > 1 else f"{span}-{page_nums[0]}"
                    else:
                        label = f"{min(page_nums)}〜{max(page_nums)}" if len(page_nums) > 1 else f"{page_nums[0]}"
                else:
                    label = f"{span}径間・p.{pp+1}" if (is_multi_span and span) else f"p.{pp+1}"
        ...
        for pp, dlist in photo_to_diag_pages.items():
            ...
            btn_list = []
            page_span = photo_span.get(pp)
            for didx in dlist:
                if is_multi_span and page_span:
                    back_label = f"{page_span}径間・p.{didx+1}"
                else:
                    back_label = f"p.{didx+1}"
...
class App(tk.Tk if not HAS_DND else TkinterDnD.Tk):
    ...
    def __init__(self):
        ...
        self._dpi         = tk.IntVar(value=150)  # デフォルト150dpi
    ...
    def _build_ui(self):
        ...
        for val, label in [(100, "低速・粗"), (150, "標準（推奨）"), (200, "高精度"), (250, "最高精度")]:
            rb = tk.Radiobutton(
